...
class MultiboxLoss(object):
    ...
    def _sigmoid_loss(self, y_true, y_pred):
        # y_pred = 1 / (1 + tf.exp(-y_pred))
        # y_pred = tf.maximum(tf.minimum(y_pred, 1 - 1e-15), 1e-15)
        # sigmoid_loss = -tf.reduce_sum(y_true * tf.log(y_pred) + (1 - y_true) * tf.log(1 - y_pred),
        #                               axis=-1)# / tf.to_float(tf.shape(y_pred)[0])
        # return sigmoid_loss
        sigmoid_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true, logits=y_pred)
        sigmoid_loss = tf.reduce_sum(sigmoid_loss, axis=-1)
        return sigmoid_loss
    def compute_loss(self, y_true, y_pred):
        ...
        if self.num_classes == 2:
            conf_loss = self._sigmoid_loss(y_true[:, :, 4:-8],
                                           y_pred[:, :, 4:-8])
        else:
