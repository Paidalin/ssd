...
class VideoTest(object):
    ...
    def run(self, video_path = 0, start_frame = 0, conf_thresh = 0.6):
        ...
        size = (767, 432)
        videoWriter = cv2.VideoWriter('out.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 24.0, size)
        if not vid.isOpened():
...
#         vidaz = float(skv.height)/self.input_shape[0]
        vidar = float(1280)/720
        vidaz = float(720)/self.input_shape[0]
        ...
        while True:
            ...
            if len(results) > 0 and len(results[0]) > 0:
                ...
                for i in range(top_conf.shape[0]):
                    ...
                    pic = cv2.rectangle(to_draw, text_top, text_bot, self.class_colors[class_num], -1)
            ...
            # print(to_draw.shape)
            videoWriter.write(to_draw)
            # cv2.imwrite('1.png',to_draw)
